...
def junction_correction(cropAx, parametrized_skel, main_junction_coordinates, 
    ...
    if not main_junction_coordinates:
...
def object_analysis(obj, skel):
    ...
    if not skel or len(skel) < 2:  # Ensure skeleton has branches
        return [obj], [skel] if skel else [[]]
    ...
    for s in sub_skeletons:
        ...
        for junction_coordinate in junction_coordinates:
            st_cross_sections, interpolated_skel, sub_skeleton = junction_correction(rec_obj, sub_skeleton, junction_coordinate, 
                                                                   g_radius=4, g_res=0.2, H_th=0.95, shift_impose=1, Euler_step_size=0.3)
            
            
            interpolated_skel = np.array(interpolated_skel)               
            filled_cs = filling_cross_sections(st_cross_sections, g_radius=4, g_res=0.2)                
            rec_obj = object_decomposition(rec_obj, interpolated_skel, filled_cs, g_radius=4, g_res=0.2)
